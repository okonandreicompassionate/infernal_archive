...
function applyProviderConfig(
  provider: "groq" | "gemini",
  rawEntries: unknown,
  options?: { setActiveProvider?: boolean },
) {
...
function inferCharacterCountFromText(text: string) {
...
function makeEntityId(col: string) {
...
Characters: ${JSON.stringify((archive.characters || []).map((character: any) => ({ name: character.name, codeName: character.codeName, species: character.species })))}
Teams: ${JSON.stringify((archive.teams || []).map((team: any) => ({ name: team.name, type: team.type })))}
Existing relationships: ${JSON.stringify((archive.relationships || []).slice(0, 200))}

User's raw notes:
${rawText}`;
    const describeCharacterFromPrompt = (source: string, index: number) => {
      ...
      const majorAbilities =
        abilityMatch?.[1]?.trim() ||
        "Adaptive improvisation and instinctive problem solving";
      ...
      return {
        ...
        origin,
        majorAbilities:
          majorAbilities.charAt(0).toUpperCase() + majorAbilities.slice(1),
        ...
        personality:
          personalityTrait.charAt(0).toUpperCase() + personalityTrait.slice(1),
        appearance:
          "Distinctive silhouette with practical gear and a strong visual identity",
        affiliation,
        currentStatus: "DRAFT",
        suggestions: [
    ...
    const buildLocalCharacterDrafts = (sourceText: string) => {
      ...
      const selected = [...candidates]
        .slice(0, targetCount)
        .map((chunk, index) => describeCharacterFromPrompt(chunk, index));
    ...
    let parsed: any;
    const hasLiveAiKeys =
      getApiKeyPool("groq").length > 0 || getApiKeyPool("gemini").length > 0;
    ...
    if (characters.length < targetCount) {
      if (hasLiveAiKeys) {
        try {
          ...
          const completionCharacters = Array.isArray(completion.characters)
    ...
    res.json({
      characters: characters.slice(0, targetCount).map((character: any) => ({
        ...
        suggestions: Array.isArray(character.suggestions)
          ...
          : [],
        relationSuggestions: Array.isArray(character.relationSuggestions)
          ? character.relationSuggestions.map((relation: any) => ({
    ...
    res.status(500).json({
      error:
        error instanceof Error
          ? error.message
          : "Character drafts could not be generated.",
    });
  }
});
